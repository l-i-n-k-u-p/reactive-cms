...
export default {
    ...
    methods: {
        getMedia: function() {
            this.mediaFiles.page(this.currentPage).fetch()
            .then(data => {
        ...
        onSelectOption: function(option) {
            ...
            .then(responses => {
                ...
                if(!responses) {
                    this.$eventHub.$emit('dashboard-app-error', 'Some mediaFiles it doesn\'t '+typeAction)
                    return
                }
                this.$eventHub.$emit('dashboard-app-success', success+' files/s '+typeAction)
            })
            .catch(data => {
                this.$eventHub.$emit('dashboard-app-error', data.message)
            })
